...
class UserService {
	 ...
	 *
	 * This method finds a user by their username, retrieves their metadata, and generates an access token.
	 * If the user is not found, it throws an error indicating that the user is not registered.
	 *
	 * @param {object} data - The data containing the email, id, displayName, and photoUrl of the user.
	 * @returns {Promise<Object>} - A promise that resolves to the user object with an access token.
	 * @throws {Error} - Throws an error if the user is not found.
	 */
	static async googleByPass(data) {

		// we should receive in data the email, id, displayName and photoUrl
		const { email, id, displayName, photoUrl } = data;

		if(!email) throw Error('Missing email');
		if(!id) throw Error('Missing id');
		if(!displayName) throw Error('Missing displayName');

		let user = await primate.prisma.user.findUnique({
			where: {
				username: email,
			},
		});
